...
abstract contract TestBase is Test {
    ...
    // Call at the start of any concrete test's setUp. Creates four deterministic actors.
    ...
    // Deploy a FixedVault against the current `usdc`.
    function _deployFixed() internal returns (FixedVault) {
        return new FixedVault(usdc);
    }

    // Deploy a P2PTransfer against the current `usdc`.
    ...
    // One-call approve + deposit into a FlexibleVault from `user`. Returns shares minted.
    ...
    // One-call approve + lock into a FixedVault from `user`. Returns positionId.
    ...
    // One-call approve + send via a P2PTransfer from `user`.
    ...
    // Advance chain time by N seconds.
